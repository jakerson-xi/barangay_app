...
<div class="content my-3">
    ...
    <table id="resident" class="table table-bordered table-hover mt-3" style="width:100%">
        ...
        <tbody>
            ...
            @else
            <p hidden>{{$fullname = $request->first_name." ".$request->middle_name." ".$request->last_name}}</p>

            @endif
            <tr>
                <td style="text-transform: uppercase;">{{$request->reference_key}}</td>
                <td style="text-transform: uppercase;">{{$request->request_type_name}}</td>
                <td style="text-transform: uppercase; "><span class="d-inline-block text-truncate" style="max-width: 170px;">
